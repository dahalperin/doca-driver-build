...
func (d *driverMgr) unmountRootfs(ctx context.Context) error {
	...
	if mountCount > 1 {
		...
		_, _, err := d.cmd.RunCommand(ctx, "umount", "-l", "-R", d.cfg.MlxDriversMount)
		if err != nil {
			// Non-fatal error, log and continue
			log.V(1).Info("Failed to unmount", "mount", d.cfg.MlxDriversMount, "error", err)
		}
		...
		if err := d.os.RemoveAll(removePath); err != nil {
			// Non-fatal error, log and continue
			log.V(1).Info("Failed to remove directory", "path", removePath, "error", err)
		}
	}
